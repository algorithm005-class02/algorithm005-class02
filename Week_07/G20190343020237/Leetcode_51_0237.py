...
class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        ...
        # # Your memory usage beats 74.26 % of python3 submissions(13.1 MB)

        # ans = []
        # y_valid, xy_sum_valid, xy_dif_valid = [True] * n, [True] * (2 * n - 1), [True] * (2 * n - 1)
        # path = []
        ...
        # Your memory usage beats 56.78 % of python3 submissions (13.3 MB)

        ans = []

        def dfs_helper(path, cols, pies, nas):
            if len(path) == n:
                ans.append(['.' * e + 'Q' + '.' * (n - e - 1) for e in path])
                return
            ...
            for i in range(n):
                if ((bits >> i) & 1) == 1:
                    p = (1 << i)
                    dfs_helper(path + [i], cols | p, (pies | p) << 1, (nas | p) >> 1)
        dfs_helper([], 0, 0, 0)
        return ans
